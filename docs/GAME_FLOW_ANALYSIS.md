# CyberQuest — Complete Game Flow Analysis

> **Generated by:** Static analysis of all 33 scene files + engine code  
> **Scope:** Every `loadScene()`, `setStoryPart()`, `setFlag()`, `getFlag()`, hotspot transition, and `onEnter`/`onExit` callback  
> **Note:** Sections 3–10 were written against the original 23 scenes. The 10 scenes added in v1.1 (see below) are noted in the reachability table but not yet fully decomposed.
...
### ✅ All 33 Scenes Are Reachable

23 scenes are fully analysed below. The 10 scenes added in v1.1 are listed
with their known incoming transitions; full breakdown pending.
...
| epilogue | debrief |
| credits | epilogue |
| **garden_back** *(v1.1)* | garden (back-garden hotspot) |
| **usb_discovery** *(v1.1)* | klooster / car_discovery (USB pickup) |
| **hackerspace** *(v1.1)* | garden/mancave via driving_day (hackerspace destination) |
| **hackerspace_classroom** *(v1.1)* | hackerspace |
| **drone_hunt** *(v1.1)* | facility approach (pre-infiltration) |
