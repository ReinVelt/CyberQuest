...
const PlanetenpadScene = {
    ...
    _walkPhases: [
        // Phase 0: Entrance — Pluto & the outer darkness
        {
            ...
            narration: [
                { speaker: '', text: '*A narrow sandy path winds through the Drenthe heathland. A wooden sign at the trailhead reads:*' },
                { speaker: '', text: '*PLANETENPAD — Schaalmodel van ons Zonnestelsel — Wandel van Pluto naar de Zon — 1 kilometer*' },
                { speaker: '', text: '*The air smells of warm heather and pine resin. A skylark trills somewhere overhead.*' },
        ...
        // Phase 1: Walk to Pluto marker
        {
            ...
            narration: [
                { speaker: '', text: '*A few steps in. The first pedestal appears — barely visible. A tiny sphere, smaller than a grain of rice.*' },
                { speaker: '', text: '*A green information board reads: PLUTO — 985 m van de Zon*' },
        ...
        // Phase 2: Walking the outer void — Neptune
        {
            ...
            narration: [
                { speaker: '', text: '*The heather stretches out on both sides. Purple blooms sway gently in the wind. Birch trees rustle overhead.*' },
                { speaker: '', text: '*After a long stretch of empty path, another pedestal. A blue-tinted sphere.*' },
                { speaker: '', text: '*🔵 NEPTUNE — 750 m from the Sun*' },
        ...
        // Phase 3: Uranus & the swaying heather
        {
            ...
            narration: [
                { speaker: '', text: '*The path curves slightly. A butterfly drifts past, orange wings catching the light.*' },
                { speaker: '', text: '*Another pedestal emerges from the waist-high heather.*' },
                { speaker: '', text: '*🔵 URANUS — 480 m from the Sun*' },
        ...
        // Phase 4: Saturn — the showpiece
        {
            ...
            narration: [
                { speaker: '', text: '*The gaps between planets are shrinking. A Scots pine towers beside the path, its trunk rough with bark.*' },
                { speaker: '', text: '*A golden sphere with delicate rings — the most detailed model on the path.*' },
                { speaker: '', text: '*🪐 SATURN — 238 m from the Sun*' },
        ...
        // Phase 5: The sundial & Jupiter
        {
            ...
            narration: [
                { speaker: '', text: '*A stone sundial is set beside the path. The gnomon casts a sharp shadow across roman numerals.*' },
                { speaker: 'Ryan', text: 'Ancient tech. Still accurate. No batteries required.' },
                { speaker: '', text: '*Further along — the largest planet model. About the size of a walnut, with painted bands.*' },
                { speaker: '', text: '*🟠 JUPITER — 130 m from the Sun*' },
        ...
        // Phase 6: Gravity experiment & the wooden bench
        {
            ...
            narration: [
                { speaker: '', text: '*An interactive panel stands between the gas giants and inner planets. A spring scale with planet-marked indicators.*' },
                { speaker: '', text: '*🔬 EXPERIMENT: HOW MUCH DO YOU WEIGH ON OTHER PLANETS?*' },
                { speaker: 'Ryan', text: 'On Jupiter, I\'d weigh over 150 kilos. My knees hurt already.' },
                { speaker: '', text: '*A weathered wooden bench offers a view across the entire path. Ryan pauses.*' },
        ...
        // Phase 7: Mars, Earth, Venus, Mercury — the inner rush
        {
            ...
            narration: [
                { speaker: '', text: '*Now the planets come fast. Mars — a red pinhead. Earth — two millimetres of blue-green.*' },
                ...
                { speaker: '', text: '*Venus — yellowish, almost identical to Earth in size. Mercury — a dark speck with craters.*' },
        ...
        // Phase 8: Light speed experiment
        {
            ...
            narration: [
                { speaker: '', text: '*An information panel with LED strips along the ground.*' },
                { speaker: '', text: '*🔬 EXPERIMENT: HOW FAST IS LIGHT? — At this scale, light travels 5 cm per second.*' },
        ...
        // Phase 9: The Sun — arrival
        {
            ...
            narration: [
                { speaker: '', text: '*At the end of the path — a bright golden sphere on a sturdy pedestal. About the size of a beach ball. It seems to glow in the afternoon light.*' },
                { speaker: '', text: '*☀️ THE SUN — diameter 23 cm | Real: 1,392,000 km — 99.86% of all mass in the solar system.*' },
                ...
                { speaker: '', text: '*Beyond the sun model, the WSRT dishes are visible through the trees — 14 parabolic ears listening to the void of space this path just walked through.*' },
...
`;
    ...
    _finishCinematicWalk(game) {
        ...
        this._schedule(() => {
            ...
            this._schedule(() => {
                ...
                game.startDialogue([
                    { speaker: '', text: '*Ryan stands beside the sun model. The WSRT dishes loom beyond the trees.*' },
                    { speaker: 'Ryan', text: 'What a walk. A kilometre of perspective.' },
                    { speaker: '', text: '*The path back leads to the parking area. Or there\'s a trail forward toward the WSRT.*' },
                ]);
            }, 1500);
        }, 3000);
    },
    ...
    hotspots: [
        // ── Pluto & Kuiper Belt marker (path entrance) ──
        {
            ...
            action: function(game) {
                ...
                game.startDialogue([
                    { speaker: '', text: `*A small pedestal with a tiny sphere — barely visible. A sign reads: "${p.name}"*` },
                    { speaker: '', text: `⚪ PLUTO — ${p.dist} from the Sun model | Real distance: ${p.real}` },
                    { speaker: '', text: `Model size: ${p.size} | ${p.fact}` },
        ...
        // ── Neptune ──
        {
            ...
            action: function(game) {
                ...
                game.startDialogue([
                    { speaker: '', text: `*A blue-tinted sphere on a dark pedestal*` },
                    { speaker: '', text: `🔵 NEPTUNE — ${p.dist} from the Sun model | Real distance: ${p.real}` },
                    { speaker: '', text: `Model size: ${p.size} | ${p.fact}` },
        ...
        // ── Uranus ──
        {
            ...
            action: function(game) {
                ...
                game.startDialogue([
                    { speaker: '', text: `*A pale blue sphere on a tilted pedestal*` },
                    { speaker: '', text: `🔵 URANUS — ${p.dist} from the Sun model | Real distance: ${p.real}` },
                    { speaker: '', text: `Model size: ${p.size} | ${p.fact}` },
                    { speaker: 'Ryan', text: 'Tilted 98 degrees. Rolling around the Sun like a bowling ball.' },
                ]);
            }
        ...
        // ── Saturn ──
        {
            ...
            action: function(game) {
                ...
                game.startDialogue([
                    { speaker: '', text: `*A golden sphere with delicate rings — the most detailed model on the path*` },
                    { speaker: '', text: `🪐 SATURN — ${p.dist} from the Sun model | Real distance: ${p.real}` },
                    { speaker: '', text: `Model size: ${p.size} (with rings ~4 cm) | ${p.fact}` },
        ...
        // ── Jupiter ──
        {
            ...
            action: function(game) {
                ...
                game.startDialogue([
                    { speaker: '', text: `*The largest planet model on the path — about the size of a walnut*` },
                    { speaker: '', text: `🟠 JUPITER — ${p.dist} from the Sun model | Real distance: ${p.real}` },
                    { speaker: '', text: `Model size: ${p.size} | ${p.fact}` },
                    { speaker: 'Ryan', text: 'The king of the planets. Even the Sun dances to its tune.' },
                ]);
            }
        ...
        // ── Gravity Experiment ──
        {
            ...
            action: function(game) {
                game.startDialogue([
                    { speaker: '', text: '*An interactive panel between the gas giants and inner planets*' },
                    { speaker: '', text: '🔬 EXPERIMENT: HOW MUCH DO YOU WEIGH ON OTHER PLANETS?' },
                    { speaker: '', text: '• Moon: 16.6% of Earth — Mars: 37.6% — Jupiter: 252.8% — Sun: 2,800%' },
                    { speaker: 'Ryan', text: 'On Jupiter, I\'d weigh over 150 kilos. My knees hurt already.' },
                ]);
            }
        },
        ...
        // ── Mars ──
        {
            ...
            action: function(game) {
                ...
                game.startDialogue([
                    { speaker: '', text: `*A tiny red sphere. Barely a millimetre across.*` },
                    { speaker: '', text: `🔴 MARS — ${p.dist} from the Sun model | Real distance: ${p.real}` },
                    { speaker: '', text: `Model size: ${p.size} | ${p.fact}` },
        ...
        // ── Earth & Moon ──
        {
            ...
            action: function(game) {
                ...
                game.startDialogue([
                    { speaker: '', text: `*Two tiny spheres — one blue-green, one grey, mounted 6.4 cm apart*` },
                    { speaker: '', text: `🌍 EARTH — ${p.dist} from the Sun model | Real distance: ${p.real}` },
                    { speaker: '', text: `Model size: ${p.size} | ${p.fact}` },
        ...
        // ── Venus ──
        {
            ...
            action: function(game) {
                ...
                game.startDialogue([
                    { speaker: '', text: `*A yellowish sphere, almost identical in size to the Earth model*` },
                    { speaker: '', text: `🟡 VENUS — ${p.dist} from the Sun model | Real distance: ${p.real}` },
                    { speaker: '', text: `Model size: ${p.size} | ${p.fact}` },
        ...
        // ── Mercury ──
        {
            ...
            action: function(game) {
                ...
                game.startDialogue([
                    { speaker: '', text: `*A dark, cratered sphere. Smallest of the planetary models.*` },
                    { speaker: '', text: `⚫ MERCURY — ${p.dist} from the Sun model | Real distance: ${p.real}` },
                    { speaker: '', text: `Model size: ${p.size} | ${p.fact}` },
        ...
        // ── Light Speed Experiment ──
        {
            ...
            action: function(game) {
                game.startDialogue([
                    { speaker: '', text: '*An information panel with LED light strips*' },
                    { speaker: '', text: '🔬 EXPERIMENT: HOW FAST IS LIGHT? — At this scale: ~5 cm/s' },
                    { speaker: '', text: '• Sun→Earth: 8 min 20 s — Sun→Neptune: 4 h 10 min — Sun→Pluto: 5 h 28 min' },
                    { speaker: '', text: '• Nearest star at this scale: 6,800 km away (London to Lahore)' },
        ...
        // ── Sun Model ──
        {
            ...
            action: function(game) {
                game.startDialogue([
                    { speaker: '', text: '*A bright golden sphere on a sturdy pedestal. About the size of a beach ball.*' },
                    { speaker: '', text: '☀️ THE SUN — diameter 23 cm | Real: 1,392,000 km — 99.86% of all mass in the solar system' },
                    { speaker: 'Ryan', text: 'The whole walk. A kilometre. And THIS is the centre of it all.' },
        ...
        // ── Sundial Experiment ──
        {
            ...
            action: function(game) {
                ...
                game.startDialogue([
                    { speaker: '', text: '*A large stone sundial set into the ground beside the path*' },
                    { speaker: '', text: '🔬 EXPERIMENT: TELLING TIME BY THE SUN' },
        ...
        // ── Wooden bench ──
        {
            ...
            action: function(game) {
                game.startDialogue([
                    { speaker: '', text: '*A weathered wooden bench between the gas giants and inner planets. A view across the entire path.*' },
        ...
        // ── The path ──
        {
            ...
            action: function(game) {
                if (!game.getFlag('planetenpad_walked')) {
                    ...
                    game.startDialogue([
                        { speaker: '', text: '*A sandy path stretches through the heathland toward the distant WSRT dishes*' },
        ...
        // ── Back to parking ──
        {
            ...
            action: function(game) {
                game.startDialogue([
                    { speaker: '', text: '*Ryan turns back toward the parking area*' },
        ...
        // ── Continue to WSRT ──
        {
            ...
            action: function(game) {
                ...
                game.startDialogue([
                    { speaker: '', text: '*Past the sun model, the path leads straight to the WSRT*' },
    ...
    onEnter: function(game) {
        ...
        if (!game.getFlag('visited_planetenpad')) {
            ...
            this._schedule(() => {
                game.startDialogue([
                    { speaker: '', text: '*The Planetenpad stretches ahead. Click the path to walk it.*' },
            ...
            this._schedule(() => {
                game.startDialogue([
                    { speaker: '', text: '*The Planetenpad. Planet markers line the sandy path through the Drenthe heath.*' },
